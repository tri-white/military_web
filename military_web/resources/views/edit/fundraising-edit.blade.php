...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8 text-white">
            <h2 class="text-center mt-4 mb-4">Редагування збору коштів</h2>
            <form action="{{ route('edit_post-fundraising', ['userid' => Auth::user()->id, 'postid' => $post->id]) }}" method="POST">
                @csrf

                <div class="form-group mb-3">
                    <label for="purpose">Заголовок для оголошення про збір коштів:</label>
                    <input type="text" class="form-control" id="purpose" name="purpose" value="{{ old('purpose', $post->purpose) }}" required>
                </div>

                <div class="form-group">
                    <label for="goal_amount">Сума, яку необхідно зібрати (грн):</label>
                    <input type="number" class="form-control" id="goal_amount" name="goal_amount" value="{{ old('goal_amount', $post->goal_amount) }}" required>
                </div>

                <div class="form-group">
                    <label for="current_amount">Скільки вже зібрано (грн):</label>
                    <input type="number" class="form-control" id="current_amount" name="current_amount" value="{{ old('current_amount', $post->current_amount) }}" required readonly>
                </div>
                ...
                <div class="div d-flex justify-content-center">
                    <button type="submit" class="btn btn-block mt-4 text-white px-5" style="background-color: #2B2C27;">Зберегти зміни</button>
                </div>
            </form>
